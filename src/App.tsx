...
const App: React.FC = () => {
  ...
  useEffect(() => {
    ...
    // ✅ Performance: প্রথমে ৫০টা fetch, বাকি Firestore এ আছে
    const q = query(collection(db, 'movies'), limit(50));
  ...
  const displayedMovies = useMemo(() => {
    ...
    if (activeCategory === 'All') {
      ...
      return filtered.filter(m => m.category === activeCategory).slice(0, visibleCount);
    }
  }, [movies, activeCategory, visibleCount]);

  const favMovies = useMemo(() => movies.filter(m => favorites.includes(m.id)), [movies, favorites]);

  // ✅ Load More — ১৮ করে বাড়াবে
  ...
  // ✅ Category change করলে reset করো
  const handleCategoryChange = useCallback((cat: Category) => {
    setActiveCategory(cat);
    setVisibleCount(18); // reset to first page
  }, []);

  // Total filtered count (for showing "Load More" or not)
  const totalFilteredCount = useMemo(() => {
    if (!movies || movies.length === 0) return 0;
    let filtered = [...movies];
    if (activeCategory === 'All') return filtered.length;
    if (activeCategory === 'Exclusive') return filtered.filter(m => m.category === 'Exclusive' || m.isExclusive === true).length;
    if (activeCategory === 'Movies') return filtered.filter(m => m.category === 'Movies' || (m.category === 'Exclusive' && !m.episodes?.length)).length;
    if (activeCategory === 'Web Series') return filtered.filter(m => m.category === 'Web Series' || (m.episodes && m.episodes.length > 0)).length;
    if (activeCategory === 'K-Drama') return filtered.filter(m => m.category === 'K-Drama' || m.category === 'Korean Drama').length;
    return filtered.filter(m => m.category === activeCategory).length;
